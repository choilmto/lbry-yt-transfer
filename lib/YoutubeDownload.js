      ...
      (err, response) => {
        ...
        db.serialize(() => {
          ...
          if (response.hasOwnProperty('nextPageToken')) {
            ...
            getChannelVids(chid, playlistid, response.nextPageToken);
          }
          else {
...
function downChannelVids(channelId) { // Downloads all the videos from the playlist and saves them to the db and on disk for lbry upload.
  return new Promise(function (success, reject) {
    ...
    let callback = function (err, row) {
      ...
      logger.info(row);
      limiter.submit(dlvid, channelId, row, null);
    };

    let completion = function (error, count) {
      if (count === 0) {
        logger.info('All videos were already downloaded!');
...
function dlvid(chid, row, cb) {
  ...
  const video = ytdl('https://www.youtube.com/watch?v=' + row.videoid,
    ...
    // Start will be sent as a range header
    {
      start: downloaded,
      cwd: __dirname,
      maxBuffer: 1000 * 10240 //Simple hotfix for buffers
    });

  // Will be called when the download starts.
  video.on('info', info => {
    logger.info('[YoutubeDownload] : Download started for video %s', row.videoid);
  });
  ...
  video.on('end', () => {
    logger.info('[YoutubeDownload] : Download finished for video %s', row.videoid);
    cb();
    // Db edit downloaded to 1
    db.run('UPDATE videos SET downloaded=1 WHERE videoid=\'' + row.videoid + '\'');
  });
