...
LbryUpload.prototype.performSyncronization = function () {
  this.ensureDaemon()
    .then(this.getAllUnprocessedVideos)
    .then(data => {
      data.forEach(row => {
        this.setupPayload(row)
          .then(this.publish)
          .then(this.savePublish)
          .catch(logger.error);
      });
    }).catch(logger.error);
};
 ...
 */
LbryUpload.prototype.ensureDaemon= function() {
...
LbryUpload.prototype.getAllUnprocessedVideos = function() {
...
LbryUpload.prototype.setupPayload = function (row) {
  ...
  let name = this.customTag + "-" + row.videoid.replace(/[^A-Za-z0-9\-]/g, '-');
  let filePath = this.videosLocation + row.channelid + '/' + filename;
  return new Promise(function (fulfill, reject) {
 ...
 */
LbryUpload.prototype.publish = function(payloadBundle) {
...
LbryUpload.prototype.savePublish = function (publishResponse) {
  const filename = this.videosLocation + publishResponse.videodata.videoid + ".mp4"
