...
function getAllUnprocessedVideos() {
  ...
  return new Promise(function (fulfill, reject) {
    ...
    const query =
      ...
      "FROM videos WHERE downloaded = 1 AND data LIKE '%Creative Commons%' " +
...
function downloadVideo(payloadBundle) {
  ...
  const filename = "/tmp/" + payloadBundle.filename;
...
function savePublish(publishResponse) {
  const filename = "/tmp/" + publishResponse.videodata.videoid + ".mp4"
