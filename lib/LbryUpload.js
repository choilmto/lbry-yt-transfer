...
function publish(payload, filename, videodata) {
    return new Promise(function (fulfill, reject) {
        ...
        request(options, function (error, response, body) {
            ...
            if (response.statusCode !== 200 || JSON.parse(body).hasOwnProperty('error')) {
                return reject(body);
            }
            else
...
function checkDaemon() {
    return new Promise(function (fulfill, reject) {
        ...
        request(options, function (error, response, body) {
            ...
            if (JSON.parse(body)['result'].hasOwnProperty('is_running') && JSON.parse(body)['result'].is_running === true)
                return fulfill(body);
            else reject(body);
        });
...
function getAllUnprocessedVideos() {
    return new Promise(function (fulfill, reject) {
        ...
        db.all("SELECT videoid,channelid,fulltitle,description, thumbnail, data FROM videos WHERE downloaded = 1 and data like '%Creative Commons%' and videos.videoid not in (select videoid from syncd_videos) limit 10;", function (err, rows) {
            if (err) {
                return reject(error);
            }
            fulfill(rows);
