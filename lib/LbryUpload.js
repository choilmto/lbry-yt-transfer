...
function publish(payloadBundle, failures) {
  ...
  logger.info("[%s] Publishing %s - %s", new Date().toISOString(), payloadBundle.payload.params.name, payloadBundle.payload.params.title);
  return new Promise(function (fulfill, reject) {
    return lbry.publish(payloadBundle.payload.params.name, 1.0, payloadBundle.payload.params)
      ...
      .then(body => {
        if (body.hasOwnProperty('error')) {
          ...
          logger.info("[%s][YT-LBRY] Success in publishing %s", new Date().toISOString(), payloadBundle.payload.params.name);
